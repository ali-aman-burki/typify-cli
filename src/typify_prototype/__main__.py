...
class ScopeVisitor(ast.NodeVisitor):
	...
	def _record(self, line: int, col: int, identifier: str, node_type: str, params: list[str] | None = None) -> None:
		...
		if node_type == "Function":
			...
			entry["callsites"] = {
				_random_location(self._rel_paths): {
					"params": {p: _random_type() for p in (params or [])},
					"type": _random_type(),
				}
...
def main() -> None:
	...
	for i, py_path in enumerate(py_files):
		...
		try:
			out_path.write_text(json.dumps(analyze_file(py_path, rel_paths), indent="\t", ensure_ascii=False), encoding="utf-8")
