...
## Nested callee param back-propagation (Pass 3d)

### Problem
When a function `execute(a, b)` is called with `execute(1, "")` and its body calls `process(a, b)`, the callsite records for `process` captured during Pass 3 carry `UNKNOWN` argument types — because `execute`'s params `a` and `b` were not yet typed at that point. Pass 3b then correctly types `execute.params` from the outer callsite, but `process.params` stay blank.

### Fix
Pass 3c now re-records callsites (with `record_callsites=True`) after clearing the old records, so that `process(a, b)` is re-observed with `a: int, b: str` from the now-typed scope of `execute`. A new Pass 3d (`apply_callsites` with `preserve_callsite_returns=True`) unions those fresh records into `process.params`, `process.params[name]["usage"]`, and the corresponding `Parameter` entries — without overwriting the per-callsite return types that Pass 3b.5 already computed via symbolic execution.
